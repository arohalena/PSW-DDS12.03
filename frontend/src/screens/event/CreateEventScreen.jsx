            ...
            <div className="voting-config-list">
              {votingConfigs.length === 0 ? (
                <div className="feedback-card">
                  Este evento se creará sin votaciones. Después podrás entrar al evento y
                  pulsar “Nueva Votación”.
                </div>
              ) : null}

              {votingConfigs.map((config, index) => {
                const total = config.criteria.reduce(
                  (sum, criterion) => sum + Number(criterion.peso || 0),
                  0
                );
                          ...
                          {tiposVotacion.map((tipo) => (
                            <button
                              ...
                              className={`option-card ${
                                config.tipo === tipo.value ? "selected" : ""
                              }`}
                              onClick={() =>
                                updateVotingConfig(config.id, { tipo: tipo.value })
                              }
                            >
                              <Vote size={18} />
                              {tipo.label}
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateVotingConfig(config.id, {
                              maxSelecciones: event.target.value,
                            })
                          }
                        />
                      </label>
                        ...
                        {modalidades.map((modalidad) => (
                          <button
                            ...
                            className={`modalidad-card ${
                              config.modalidad === modalidad.value ? "selected" : ""
                            }`}
                            onClick={() =>
                              updateVotingConfig(config.id, {
                    ...
                    <div className="criteria-configurator">
                      ...
                      <div className="criteria-configurator-header">
                        ...
                        {config.modalidad === "MULTICRITERIO_PONDERADA" ? (
                          <div
                            className={`criteria-weight-alert ${
                          ...
                          {config.criteria.map((criterion, criterionIndex) => (
                            <div className="criterion-card" key={criterion.id}>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  removeCriterion(config.id, criterion.id)
                                }
                              >
                                <Trash2 size={17} />
                              </button>
