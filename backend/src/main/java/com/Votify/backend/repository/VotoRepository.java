...
public interface VotoRepository extends JpaRepository<VotoMO, UUID> {
    ...
    List<VotoMO> findByVotacionProyecto_Votacion_Id(UUID votacionId);

    List<VotoMO> findByVotacionProyecto_Votacion_Evento_Id(UUID eventoId);
    ...
    boolean existsByUsuario_IdAndVotacionProyecto_Votacion_Id(UUID usuarioId, UUID votacionId);
